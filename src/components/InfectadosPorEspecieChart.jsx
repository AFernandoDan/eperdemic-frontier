...
const InfectadosPorEspecieChart = () => {
    const [infectadosPorEspecie, setInfectadosPorEspecie] = useState([]);
    const [loading, setLoading] = useState(true);
  
    useEffect(() => {
      fetch("http://localhost:8080/estadistica/enfermosPorEspecie")
      .then((response) => response.json())
      .then((data) => {
        const infectadosParsed = parseData(data);
        setInfectadosPorEspecie(infectadosParsed);
        setLoading(false);
      })
  ...
  return (
    <>
        {infectadosPorEspecie.length > 0 && !loading && <Chart
            chartType="AreaChart"
            width="100%"
            height="100vh"
            data={infectadosPorEspecie}
            options={options}
        />}
    </>
  )
...
export default InfectadosPorEspecieChart